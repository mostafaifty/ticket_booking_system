...
@section('title', 'Edit Station - ' . $station->name)
@section('page_title', 'Edit Railway Station')
        ...
        <div class="card card-outline card-primary shadow-sm">
            <div class="card-header">
                <h3 class="card-title font-weight-bold">
                    <i class="fas fa-edit mr-1 text-primary"></i> Edit Station: {{ $station->name }}
                </h3>
            </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-5 mb-3">
                            ...
                            <select name="status" id="status" class="form-control @error('status') is-invalid @enderror" required>
                                <option value="active" {{ old('status', $station->status) === 'active' ? 'selected' : '' }}>Active (In Service)</option>
                                <option value="inactive" {{ old('status', $station->status) === 'inactive' ? 'selected' : '' }}>Inactive (Closed / Suspended)</option>
                            </select>
                            @error('status')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
